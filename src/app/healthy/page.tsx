...
export default async function HealthyFood() {
    const data = await SanityClient.fetch(groq` *[_type == "healthy"]{
  ...
  return (
    ...
    <div className="max-w-6xl mx-auto p-4">
      <div className="text-center">
        <h2 className="text-3xl font-extralight text-slate-400 inline-block relative after:absolute after:w-4/6 after:h-1 after:left-0 after:right-0 after:-bottom-4 after:mx-auto after:bg-slate-400 after:rounded-lg-full">Healthy Food</h2>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6 mt-16 max-lg:max-w-3xl max-md:max-w-md mx-auto">
        
       
       {data.map((food:any)=>(
      <Link href={`/healthy/${food.slug.current}`}>
